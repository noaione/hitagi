    ...
    <div class="flex flex-row items-center">
      <div class="flex flex-row items-center">
        ...
        <NuxtLink
          ...
          class="ml-2 text-hitagi-800 transition-opacity hover:opacity-80 dark:text-hitagi-200"
        >
          {{ data.title }}
        </NuxtLink>
      </div>
      <div class="flex flex-row gap-2 md:ml-2" v-if="compact">
        <LinkablePill
          :key="idx"
        ...
        <span v-if="dateAdded" class="my-1 text-sm text-hitagi-700 dark:text-hitagi-300">|</span>
        <ArchiveUnix
          :unix="dateAdded"
          text="Added on"
          v-if="dateAdded"
