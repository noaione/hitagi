<template>
  <div :class="`flex flex-col ${containerClass ?? ''}`">
    <slot name="label" />
    <input v-bind="$attrs" :id="inputId" v-model="inputModel" :class="inputFormClass" />
  </div>
</template>
...
const inputFormClass = clsx(
  "form-input rounded-md bg-white transition disabled:cursor-not-allowed disabled:bg-gray-100 dark:bg-gray-800 disabled:dark:bg-gray-900",
