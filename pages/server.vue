<template>
  <div ref="loginForm" class="font-incosolata flex h-screen flex-col items-center justify-center">
    ...
    >
      <template #label>
        <label for="api-token" class="font-medium text-themed-700 dark:text-themed-300">Server</label>
      </template>
    </HitagiInput>
    <HitagiInput
      v-if="needApiToken"
...
.btn-server-submit[data-error="network"] {
  @apply disabled:animate-pulse disabled:bg-themed-700;
}

.btn-server-submit[data-error="validate"] {
  @apply disabled:bg-themed-700;
}
</style>
